...
(function () {
    ...
    $self.buhlmann = function() {
        ...
        function buhlmannTissue(halfTimes, absPressure, isFreshWater) {
            ...
            this.pNitrogen = dive.partialPressure(1, 0.79) - this.waterVapourPressure;
            this.pHelium = 0;
        ...
        buhlmannTissue.prototype.addDepthChange = function (startDepth, endDepth, fN2, fHe, time) {
            ...
            this.pNitrogen = dive.shreinerEquation(gasRate, time, timeConstant, pGas, pBegin);
        ...
        buhlmannTissue.prototype.calculateCeiling = function () {
            ...
            this.ceiling = dive.barsToDepthInMeters(bars, this.isFreshWater);
        };

        function dive(buhlmannTable) {
            this.table = buhlmannTable;
            this.tissues = [];
            for (var i = 0; i < this.table.length; i++) {
                this.tissues[i] = new buhlmannTissue(this.table[i]);
            }
        };

        dive.prototype.addFlat = function (depth, fN2, fHe, time) {
            for (var i = 0; i < this.tissues.length; i++) {
                this.tissues[i].addFlat(depth, fN2, fHe, time);
            }
        };

        dive.prototype.addDepthChange = function (startDepth, endDepth, fN2, fHe, time) {
            for (var i = 0; i < this.tissues.length; i++) {
                this.tissues[i].addDepthChange(startDepth, endDepth, fN2, fHe, time);
            }
        };

        dive.prototype.getCeiling = function () {
            var ceiling = 0;
            for (var i = 0; i < this.tissues.length; i++) {
                var tissueCeiling = this.tissues[i].calculateCeiling();
                if (tissueCeiling > ceiling) {
                    celing = tissueCeiling;
                }
            }
        };
